...
// （VOICE_WAIT_MAX_MS を過ぎたら待たずに出し始め、声は遅れて鳴るまま）。
...
// 待たせるのは GameAudio.speak が started を返したときだけ（入っている dtm が対応しているとき）。
...
/** 声の鳴り始めを待って文字送りを止めておくいちばん長い時間（ms）。 */
const VOICE_WAIT_MAX_MS = 1000;
...
export type ShowHook = {
	 ...
	 * 声の頭まで待たせ（VOICE_WAIT_MAX_MS まで）、声の長さへ寄せる。無ければすぐ出し始める。
	 */
		...
		return new Promise((resolve) => {
			...
			if (!voice) {
				...
				void voice.then((cue) => {
					...
					voiceEnd = cue.endAt;
					// 待ちきれずに出し始めていたら、残りの文字を声の長さに合わせるだけ
					if (typing) return;
					window.clearTimeout(timer);
