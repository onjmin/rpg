...
`src/data/minors.ts`。おんJwiki の「一軍・二軍」まわりの顔文字キャラの寄り道。本編とは ゆるく からみます：話の進み（沈黙のあいだ・声がもどったあと・完走のあと）で1回だけの ひとことが出る、たたかう仲間が口をはさむ、会った子が終章のレスの洪水（>>995〜>>997）に書きこむ（`threadlog.ts` の `MINOR_POSTS`）。町の広場のおんちゃん（一軍）とにぃちぇ・スレ街道の北東のすみのおんすちゃん・スタジアムのヤヤポジとポジハメ・過去ログ倉庫の奥の間（B2 のあと）のンゴ姉とパン松。奥の間の はり紙で「一軍選抜総選挙」に1票入れられます（`vote`）。おんちゃん・ポジハメ・先住民（( ‘ｊ’ )「あ！今日土曜日ど！」）の歩行グラは RPGEN、ほかの5人は RPGEN に無かったので `scripts/make-sprites.mjs` で描いています。
