...
`src/data/minors.ts`。おんJwiki の「一軍・二軍」まわりの顔文字キャラの寄り道（本編には からまない）。町のにぃちぇ（遊んでいる端末が日曜日だとセリフが変わる）・スレ街道の北東のすみのおんすちゃん・スタジアムのヤヤポジ・過去ログ倉庫の奥の間（B2 のあと）のンゴ姉とパン松。奥の間の はり紙で「一軍選抜総選挙」に1票入れられます（`vote`）。歩行グラは `scripts/make-sprites.mjs` で描いています。
...
- おんJマイナーズ（にぃちぇ・おんすちゃん・ンゴ姉・パン松・ヤヤポジ）: おんJ のみんな（設定は [おんJwiki](https://w.atwiki.jp/openj3/) を参考に、一部を借りた非公式の創作です）
