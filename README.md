...
`src/data/minors.ts`。おんJwiki の「一軍・二軍」まわりの顔文字キャラの寄り道（本編には からまない）。町の広場のおんちゃん（一軍）とにぃちぇ・スレ街道の北東のすみのおんすちゃん・スタジアムのヤヤポジ・過去ログ倉庫の奥の間（B2 のあと）のンゴ姉とパン松。奥の間の はり紙で「一軍選抜総選挙」に1票入れられます（`vote`）。おんちゃんの歩行グラは RPGEN（`sa:oLrlUq`）、ほかの5人は `scripts/make-sprites.mjs` で描いています。
