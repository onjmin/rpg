...
- 開発中は URL でタイトルを飛ばして好きな場所から始められます（`pnpm dev` のときだけ）:
  ...
  - `&items=memo_roze:1,rec_first:1` で持ちものを決め打ちします（数を省くと 1、`:0` で なくす）。次スレでの差分を試すときに。
  - 誕生スレ（`map=thread`）から始めるときは `"done:thread:opening":true` を flags に入れてください（入れないと序章が始まります）。
...
`src/data/minors.ts`。おんJwiki の「一軍・二軍」まわりの顔文字キャラの寄り道。本編とは ゆるく からみます：話の進み（沈黙のあいだ・声がもどったあと・完走のあと）で1回だけの ひとことが出る、たたかう仲間が口をはさむ、会った子が終章のレスの洪水（>>995〜>>997）に書きこむ（`threadlog.ts` の `MINOR_POSTS`）、2回目からは その子の雑談（仲間の口出しつき）が1本ずつ出る（`chat`）。町の広場のおんちゃん（一軍）とにぃちぇ・スレ街道の北東のすみのおんすちゃん・スタジアムのヤヤポジとポジハメ・過去ログ倉庫の奥の間（B2 のあと）のンゴ姉とパン松。奥の間の はり紙で「一軍選抜総選挙」に1票入れられます（`vote`）。おんちゃん・ポジハメ・先住民（( ‘ｊ’ )「あ！今日土曜日ど！」）の歩行グラは RPGEN、ほかの5人は RPGEN に無かったので `scripts/make-sprites.mjs` で描いています。

RPGEN の歩行グラの検索（rpgen-search）は名前の**前方一致**です（「土曜日」では「あ！今日土曜日ど！」に当たらない）。見つからないときは、一覧をぜんぶ取ってから部分一致でさがすと確実です。

### 曜日連動（`src/data/weekday.ts`）

ゲームの中の日付は 2026-08-18（火曜日）のままです（先住民の「土曜日ど！」に おんJ民が「火曜日やぞ」と返す日）。それとは別に、遊んでいる端末の曜日が しみだしてくる所があります。

| 曜日 | 変わるところ |
|---|---|
| 毎日 | 勢い欄の2番目のスレタイ（B2 まで。火・土は もとの「土曜日ど！」）・おんちゃんの「今日のおんちゃん」 |
| 日 | にぃちぇ「あ！今日　日曜日だニィ！」 |
| 月 | スタジアムの観客（左上）「ほんまは　試合　ない日やで」 |
| 土 | にぃちぇ「あした　日曜日だニィ！」・先住民の「土曜日ど！」に おんJ民が言いかけて やめる |

期間限定（遊んでいる端末の日付。`season`）：正月（1/1〜1/7）・バレンタイン・エイプリルフール・七夕・ハロウィン・クリスマス（12/24〜25）・大みそかに、おんJマイナーズの ひとことが変わります（おんちゃんは「今日のおんちゃん」の かわりに言う）。開発中は URL の `&date=MMDD` で決め打ちできます。

端末の日時は `src/data/weekday.ts` の `now()` で読みます（開発中は `&date=MMDD&time=HHMM` で決め打ち）。その日だけの場面は `src/data/days.ts` の `DAYS` にあり、`pnpm validate` は決め打ちの日時で `DAYS` を全部走らせます。場面を足したら、その日時を `scripts/validate.mjs` の一覧にも足してください。

開発中は URL に `&wday=0`〜`6`（0 が日曜）を足すと曜日を決め打ちできます（`pnpm dev` のときだけ）。`pnpm validate` は その日の曜日しか通らないので、曜日の文を足したら 7曜日ぶん確かめてください（`Date.prototype.getDay` を差しかえて `scripts/validate.mjs` を走らせる、など）。

### 縛り（ふつうは しない 遊び方）

仕様では できるが ふつうは しない 遊び方で完走すると、エンディングで仲間が気づき、まとめカードの `>>998` に残ります（`src/data/threadlog.ts` の `shibari`）。

| 縛り | 見分け方 | 気づく人 |
|---|---|---|
| どうぐ縛り | 仲間が どうぐを1回も使っていない（`play_item`） | テト |
| うた縛り | 仲間が うたを1回も歌っていない（`play_song`） | ロゼ |
| 低レベル | ボツキリコに勝ったときの キリコが Lv8 以下（`f2_lv`） | おんJ民 |
| キリコひとり | ボツキリコと ひとりで たたかった（`f2_solo`） | フェリス |

2つ以上なら スレの住民（J民C）も気づきます。`play_item`・`play_song` は戦闘とメニューで数えます（`engine/party.ts` の `countPlay`）。このしくみより前のセーブは数えていないので、`play_track`（はじめからの記録にだけ立つ）が無ければ 縛りとは みなしません。
...
- **次スレ**（`src/engine/newgame.ts`）… 完走したきろくがあると、タイトルに「次スレを　立てる」が出ます。引き継ぐのは 仲間のレベル（`p2_lv_<id>`。加入時に `join` が見ます）・なかよし度・見た会話・発掘・テストサーバーの記録・大事なもの。話の進みは まっさらに戻り、敵は `p2_n` に応じて強くなります（2周目 1.6倍、3周目 2.2倍）。フラグを足すときは `p2_` で始めると `validate` の「フラグの約束」から外れます（ただし持ち越しはしません）。スレをまたいで覚えておくフラグは `keep_` で始めます（`newgame.ts` の `CARRY_PREFIX`）。
...
- フェリス・おんJ民（やきう）: なんJ・おんJ のみんな
- おんちゃん・おんJマイナーズ（にぃちぇ・おんすちゃん・ンゴ姉・パン松・ヤヤポジ）: おんJ のみんな（設定は [おんJwiki](https://w.atwiki.jp/openj3/) を参考に、一部を借りた非公式の創作です）
